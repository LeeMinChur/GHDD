                        ...
                        if len(choice_menu)>5:
                            ...
                            while True:
                                ...
                                if GPIO.input(btn_ok) == GPIO.HIGH:
                                    ...
                                    sock.sendall(order_list.encode('utf-8'))
        ...
        while True:
            ...
            if recv.decode('utf-8')=='메뉴세팅':
                ...
                while True:
                    ...
                    draw = ImageDraw.Draw(image)
                    disp.clear()
                    draw.text((x+34,top+32),'메뉴가 변경됩니다',font=font,fill=255)
                    draw.text((x+34,top+32),'6. OK ',font=font,fill=255)
                    disp.image(image)
                    disp.display()
                    time.sleep(2)
                    if GPIO.input(btn_ok) == GPIO.HIGH:
                        menu.append(recv.decode('utf-8'))
                        if recv.decode('utf-8') == '세팅완료':
                            image=Image.new('1',(width,height))
                            draw = ImageDraw.Draw(image)
                            disp.clear()
                            draw.text((x+34,top+32),'메뉴가 변경되었습니다',font=font,fill=255)
                            draw.text((x+34,top+32),'다시 주문해주세요',font=font,fill=255)
                            disp.image(image)
                            disp.display()
                            time.sleep(2)
                            break
